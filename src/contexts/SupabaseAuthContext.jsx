...
export const AuthProvider = ({ children }) => {
  ...
  // ✅ Correction : remplacement de user_has_feature par check_user_access
  const checkFeaturePermission = useCallback(async (userId, featureKey) => {
    if (!userId) return false;

    // 🔁 Correspondance entre anciens featureKey et la nouvelle logique d’accès
    const mapping = {
      'create_annonce': { section: 'annonces', action: 'create' },
      'annonces_read': { section: 'annonces', action: 'view' },
      'evenements_read': { section: 'evenements', action: 'view' },
      'faits_divers_read': { section: 'faits_divers', action: 'view' },
      'groupes_read': { section: 'groupes', action: 'view' },
      'partenaires_access': { section: 'partenaires', action: 'view' },
      'rencontre_access': { section: 'rencontre', action: 'view' },
      'create_rencontre': { section: 'rencontre', action: 'create' },
      'position_access': { section: 'position', action: 'view' }
    };

    const params = mapping[featureKey] || { section: featureKey, action: 'view' };
    ...
    if (error) {
      console.error(`Erreur check_user_access pour ${featureKey}:`, error);
      return false;
    }
